...
router.post('/classify', authenticate, async (req, res) => {
  ...
  req.io.emit('classification', { _id: history._id, text, predictions });
 ...
 *     description: Remove a history item from the user's history.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - name: id
 *         in: path
 *         required: true
 *         description: The ID of the history item to delete
 *         schema:
 *           type: string
 *     responses:
 *       204:
